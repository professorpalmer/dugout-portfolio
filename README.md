...
| **Per-username login lockout (DB-backed)** | 5 failures → 5-min lock, tracked in `failed_login_count` / `locked_until` on the `user` table. DB-backed (not per-process in-memory) so the lockout holds across both Gunicorn workers and survives container restarts. Combined with `slowapi` per-IP limits, this covers both distributed (botnet) and single-account credential stuffing |
| **Redis-backed `slowapi` storage** | Default in-memory rate-limit storage is *per-worker* — with 2 Gunicorn workers, attackers could double every limit. Pointed `slowapi` at Redis (DB 1) so counters are shared across workers and survive restarts. Required for any real rate limiting claim to be true under multi-worker deployment |
